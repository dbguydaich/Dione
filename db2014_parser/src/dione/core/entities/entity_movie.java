...
public class entity_movie{
	...
	public void set_fq_name()
	{
		...
		entity_movie_labels.add(this.get_movie_name());
		for (String label : this.entity_movie_labels)
		{
			/*label + year or just label*/
			this.entity_movie_labels_fq.add(build_fq_name(label));
			...
			{
				Set<String> directors = this.get_movie_director().get_person_names();
				directors.add(this.get_movie_director().get_person_name());
				for (String director_name : directors)
				{
	...
	/* overload - builds a soft year key*/
