 ...
 * 
 */
public abstract class abstract_yago_parser implements Iimport_task {
	 ...
	 * @return 
	 */
	public abstract int handle_line_parsing(String splitted_line[]);
	 ...
	 * specific implementation, returns the amount of lines parsed succefully
	 */
	public long parse_file() {
		long parsed_lines = 0; 
			...
			while ((splitted_line = split_and_clean_line(br,
				...
				try { 
					...
					if (is_line_parseble(splitted_line)) 
						parsed_lines += handle_line_parsing(splitted_line);
				} catch (Exception ex) { /*error parsing single line - don't report*/
					continue; 
				}
			}
			br.close();		
		} catch (Exception ex) { /*major parsing error*/
	...
	/* if forcefully terminated, we consider the task to be invalid*/
	...
	/**
	 * interface method, return 1 iff task was performed succeffully. 
	 *  in this context, we excpect parsing to handle at least one line
	 */
	...
	/**
	 * Interface method. we regard a parsing task size, as the amount 
	 * of lines that need be parsed in the file
	 */
