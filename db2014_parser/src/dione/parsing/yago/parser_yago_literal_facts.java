...
/**
 * class for parsing yago literal facts, such as years, lengths
 * @author GUY
 *
 */
public class parser_yago_literal_facts extends abstract_yago_parser {
	...
	private String parse_movie_length(String literal_fact_length) {
		try {
			String length = literal_fact_length.substring(1,
					literal_fact_length.indexOf("."));
	...
	public void print_parse_stats() {
		try {
			...
			System.out
					.println("Error Generating Parse message for literal_facts");
		}
	}

	@Override
	public boolean is_line_parseble(String[] splitted_line) {
		if (splitted_line[yago_literal_fact_offset].contains(properties
				.get_yago_tag_year())
				|| splitted_line[yago_literal_fact_offset].contains(properties
						.get_yago_tag_length()))
			return true;
		return false;
	}
